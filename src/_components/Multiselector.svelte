...
<script>
  ...
  export let updatePopup = () => {
    if (menuPopup) menuPopup.scheduleUpdate();
  };
...
<style>
  ...
  button.dropdown:hover {
    ...
    box-shadow: 4px 4px 0px rgb(248, 178, 178), 0px 0px 10px rgba(0, 0, 0, 0.25);
  }
  ...
  .menu-content li {
    ...
    cursor: pointer;
    display: grid;
    grid-template-columns: [status] var(--multiselector-list-item-size) [content] auto [right] var(
        --multiselector-list-item-size
      );
  ...
  .menu-section {
    ...
    padding-left: calc(
      var(--multiselector-list-item-size) + var(--multiselector-list-item-pad) *
        1.5
    );
  }
