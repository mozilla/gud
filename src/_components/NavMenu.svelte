<script>
  ...
  let currentlySelected = $setter
    ? options.findIndex(o => o.key === $setter)
    : 0;
...
{#if visible}
  <h2
    in:fly={{ y: 10, duration: 300, delay: D }}
    class:smaller
    class="menu-label">
    {label}
  </h2>
  <div in:fly={{ y: 10, duration: 300, delay: 100 + D }} class="selector">
    <div class="selection-menu">
      <select class:smaller bind:value={currentlySelected}>
        <optgroup {label}>
      ...
      <div class="faker">{currentlySelectedText}</div>
    </div>
    <div in:fly={{ y: 10, duration: 300, delay: 150 + D }} class="tooltip">
      ?
    </div>
  </div>
{/if}
