...
<script>
  ...
  function getGraphWidth(size) {
    const value = getCSSVariable(
      size === "small" ? "--small-graph-width" : "--large-graph-width"
    );
  ...
  const inBounds = (xmin, xmax) => {
    ...
    return d => {
      return (
        (xmin !== "" ? d.date >= xmindate : true) &&
        (xmax !== "" ? d.date <= xmaxdate : true)
      );
    };
  };
  ...
  const H = size === "small" ? W * 0.625 : W * 0.5;
  ...
  $: filteredMarkers = markers.filter(marker =>
    filterMarkerCallback(marker, graphXMin, graphXMax)
  );

  let showReleaseMarkersOnHover = true;
  $: showReleaseMarkersOnHover =
  ...
  $: path = finalData.map(
    segment =>
      `M${segment.map(p => `${xScale(p.date)},${yScale(p.value)}`).join("L")}`
  );
  // for whatever reason, ciArea doesn't update gracefully,.
  $: ciArea = finalData.map(segment =>
    areaShape(segment.filter(d => d.date <= graphXMax && d.date >= graphXMin))
  );
...
{#if data.length}
  <div class="graphic-container">
    ...
    <svg
      ...
      in:fly={{ y: 4 * Math.sign(Math.random() - 0.5), duration: 100, delay: 150 }}
      width="100%"
      viewbox="0 0 {W}
      {H}"
      ...
      <g class="plot-background">
        {#if mouseVersionValue && mouseVersionValue.end && mouseVersionValue && showReleaseMarkersOnHover}
          ...
          <text
            in:fade={{ duration: 100 }}
            x={size === 'large' ? mouseVersionValue.start + (mouseVersionValue.end - mouseVersionValue.start) / 2 : Math.max(mouseVersionValue.start - M.buffer, PL.left + M.buffer)}
            y={PL.bottom}
            font-size="10"
            opacity=".5"
            text-anchor={size === 'large' ? 'middle' : mouseVersionValue.start - M.buffer < PL.left + M.buffer ? 'start' : 'end'}>
            {parseInt(mouseVersionValue.version)}
          </text>
        {/if}
        {#if isDragging}
      ...
      <g class="body-rollover">
        <line
          x1={$coords.x}
          x2={$coords.x}
          y1={yScale(yPoint.lower)}
          y2={yScale(yPoint.upper)}
          stroke="black" />
        <line
          x1={$coords.x - 3}
          x2={$coords.x + 3}
          y1={yScale(yPoint.upper)}
          y2={yScale(yPoint.upper)}
          stroke="black" />
        <line
          x1={$coords.x - 3}
      ...
      <text opacity=".6" font-size="10" text-anchor="start" x={PL.left} y={26}>
        {#if mouseYLow !== undefined}
          <tspan font-weight="bold" fill="black">L</tspan>
          <tspan fill="rgb(0,0,0,.7)">{`   ${yFormat(mouseYLow)}   `}</tspan>
        {/if}
        {#if mouseYHigh !== undefined}
          <tspan font-weight="bold">H</tspan>
          <tspan fill="rgb(0,0,0,.9)">{`   ${yFormat(mouseYHigh)}   `}</tspan>
        {/if}
      </text>
      ...
      <text
        ...
        {#if mouseVersionValue !== undefined && yPoint !== undefined}
          <tspan>
            {`    ${parseInt(mouseVersionValue.version)} – day ${daysBetween(yPoint.date, mouseVersionValue.date)}   `}
          </tspan>
        {/if}
      </text>
    </svg>
  </div>
{:else}Yes, there's no data with this range specified{/if}
