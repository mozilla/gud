...
import { 
    ...
    addAnnotation } from '../stores/annotations'
...
{#if active}
    <div class='modal-container' transition:fade={{duration:100}}>
        <div class='modal annotation-modal'>
            ...
            <p class=modal-body>
                These annotations get saved in the URL querystring above. If you copy and paste it and send it to someone,
                they will see your annotations. But if they do not get your exact URL above, they won't see anything.   
            </p>
            <textarea class:over-limit={!isUnderCharacterLimit} bind:value={annotation} />

            <div class=annotation-status>
                <div class:over-limit={!isUnderCharacterLimit} class=annotation-length>{howManyCharacters}</div>
                <div>
                    {#if errorMessage}
                        <div class=annotation-error-message transition:fade={{y:20, duration: 200}}>
                            {errorMessage}
                        </div>
                    {/if}
                </div>
            ...
            <div class=modal-buttons>
                <button class='bt bt-text' on:click={deactivateAnnotationModal}>cancel</button>
                <button class='bt' on:click={() => {
